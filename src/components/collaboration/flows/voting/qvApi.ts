...
export async function addProposal(serverUrl: string, publicKey: string, contractId: string, text: string) {
  return throwIfContractError(await contractWrite({ serverUrl, publicKey, contractId, method: { name: 'add_proposal', values: { text } } as IMethod }));
}

export async function setCredits(serverUrl: string, publicKey: string, contractId: string, credits: number) {
  return throwIfContractError(await contractWrite({ serverUrl, publicKey, contractId, method: { name: 'set_credits', values: { credits } } as IMethod }));
}

export async function setStatus(serverUrl: string, publicKey: string, contractId: string, status: string) {
  return throwIfContractError(await contractWrite({ serverUrl, publicKey, contractId, method: { name: 'set_status', values: { status } } as IMethod }));
}
...
export async function allocate(serverUrl: string, publicKey: string, contractId: string, allocations: Record<string, number>) {
  return throwIfContractError(await contractWrite({ serverUrl, publicKey, contractId, method: { name: 'allocate', values: { allocations } } as IMethod }));
}
